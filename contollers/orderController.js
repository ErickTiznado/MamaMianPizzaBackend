...
// Function to create order with integrated Wompi 3DS payment
/**
 * Create order with integrated Wompi 3DS payment
 * This function creates an order and immediately processes payment with Wompi
 */
exports.createOrderWithWompiPayment = async (req, res) => {
    const requestId = `WOMPI-ORDER-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    console.log(`\n[${new Date().toISOString()}] ===== INICIO PEDIDO CON PAGO WOMPI =====`);
    console.log(`🆔 Request ID: ${requestId}`);
    ...
    try {
        ...
        const {
            // Order data (same as regular order creation)
            tipo_cliente,
            cliente,
            direccion,
            productos,
            ...
            // Payment data for Wompi
            payment_data: {
                numeroTarjeta,
                cvv,
                mesVencimiento,
                anioVencimiento,
                // Customer payment info (can override order customer info)
                nombre_pago = cliente?.nombre,
                apellido_pago = cliente?.apellido,
                email_pago = cliente?.email,
                telefono_pago = cliente?.telefono,
                ciudad_pago = direccion?.ciudad || 'San Salvador',
                direccion_pago = direccion?.direccion,
                idPais = 'SV',
                idRegion = 'SV-SS',
                codigoPostal = '01101'
            }
        ...
        console.log(`📝 [${requestId}] Datos recibidos - Total: $${total}, Cliente: ${cliente?.nombre}`);

        // Validate required fields
        const errors = [];
        
        // Order validation (reuse existing validation logic)
        if (!tipo_cliente) errors.push('tipo_cliente es requerido');
        if (!cliente) errors.push('cliente es requerido');
        if (!direccion) errors.push('direccion es requerida');
        if (!productos || productos.length === 0) errors.push('productos son requeridos');
        if (!total || total <= 0) errors.push('total debe ser mayor a 0');
        if (!aceptado_terminos) errors.push('debe aceptar términos y condiciones');
        ...
        console.log(`🛒 [${requestId}] Creando pedido...`);

        // Step 1: Create the order (similar to existing createOrder function)
        const codigo_pedido = generateOrderCode();
        let id_usuario = null;
        let id_usuario_invitado = null;
        let id_direccion = null;

        // Handle user and address creation (reuse existing logic)
        if (tipo_cliente === 'registrado') {
            const [users] = await connection.query(
                'SELECT * FROM usuarios WHERE correo = ?', 
                [cliente.email]
            ...
            // Create address for registered user
            if (direccion.tipo === 'formulario') {
            ...
            if (existingGuest.length > 0) {
                ...
                const [guestResult] = await connection.query(
                    'INSERT INTO usuarios_invitados (nombre, apellido, correo, celular) VALUES (?, ?, ?, ?)',
                    [cliente.nombre, cliente.apellido, cliente.email, cliente.telefono]
                );
                id_usuario_invitado = guestResult.insertId;
            }

            // Create temporary user for address linking
            const [tempUserResult] = await connection.query(
                'INSERT INTO usuarios (nombre, correo, password, tipo_cuenta) VALUES (?, ?, ?, ?)',
                [cliente.nombre, cliente.email || `guest_${Date.now()}@temp.com`, 'temp', 'invitado']
            );
            id_usuario = tempUserResult.insertId;
        ...
        // Create order with wompi_3ds payment method
        const orderQuery = `INSERT INTO pedidos (
            codigo_pedido, id_usuario, id_direccion, estado, total, tipo_cliente, 
            metodo_pago, nombre_cliente, apellido_cliente, telefono, email, 
            subtotal, costo_envio, impuestos, aceptado_terminos, tiempo_estimado_entrega
            ${tipo_cliente === 'invitado' && id_usuario_invitado ? ', id_usuario_invitado' : ''}
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?${tipo_cliente === 'invitado' && id_usuario_invitado ? ', ?' : ''})`;

        const orderValues = [
            codigo_pedido, id_usuario, id_direccion, 'pendiente_pago', total, tipo_cliente, 
            'wompi_3ds', cliente.nombre, cliente.apellido, cliente.telefono, cliente.email || null, 
            subtotal, costo_envio, impuestos, aceptado_terminos ? 1 : 0, tiempo_estimado_entrega
        ];

        if (tipo_cliente === 'invitado' && id_usuario_invitado) {
            orderValues.push(id_usuario_invitado);
        }

        const [orderResult] = await connection.query(orderQuery, orderValues);
        const id_pedido = orderResult.insertId;

        console.log(`✅ [${requestId}] Pedido creado con ID: ${id_pedido}`);

        // Step 2: Add order details
        const detallesInsertados = [];
        for (const producto of productos) {
            try {
                // Find existing product (don't create new ones)
                const [existingProducts] = await connection.query(
                    'SELECT id_producto FROM productos WHERE titulo = ? LIMIT 1',
                    [producto.nombre_producto]
                );

                if (existingProducts.length === 0) {
                    console.warn(`⚠️ [${requestId}] Producto no encontrado: ${producto.nombre_producto}, saltando...`);
                    continue;
                }

                const id_producto = existingProducts[0].id_producto;
                ...
                console.error(`❌ [${requestId}] Error procesando producto ${producto.nombre_producto}:`, detailError.message);
            }
        }
        ...
        console.log(`💳 [${requestId}] Iniciando proceso de pago con Wompi...`);

        // Step 3: Create Wompi 3DS transaction
        const { generateAuthHeader, generateTransactionReference, formatAmount, WOMPI_CONFIG } = require('../config/wompiConfig');
        ...
        const wompiPayload = {
            ...
            monto: formatAmount(total),
            configuracion: {
                emailsNotificacion: email_pago,
                urlWebhook: WOMPI_CONFIG.URLS.WEBHOOK,
                telefonosNotificacion: telefono_pago,
                notificarTransaccionCliente: true
            },
            urlRedirect: `${WOMPI_CONFIG.URLS.REDIRECT_SUCCESS}?ref=${transactionReference}&order=${codigo_pedido}`,
            nombre: nombre_pago,
            apellido: apellido_pago,
            email: email_pago,
            ciudad: ciudad_pago,
            ...
            datosAdicionales: {
                orderId: id_pedido.toString(),
                orderCode: codigo_pedido,
                transactionReference: transactionReference,
                source: 'mama_mian_pizza_integrated'
            }
        };
        ...
        await connection.query(
            'UPDATE pedidos SET payment_reference = ? WHERE id_pedido = ?',
            [transactionReference, id_pedido]
        );
        ...
        console.log(`🎉 [${requestId}] Pedido y pago creados exitosamente`);
        console.log(`⏱️ [${requestId}] Tiempo total: ${processingTime}ms`);

        // Return success response with payment URL
        res.status(201).json({
            success: true,
            message: 'Pedido creado y pago iniciado exitosamente',
            data: {
                // Order information
                orderId: id_pedido,
                orderCode: codigo_pedido,
                orderTotal: total,
                productsProcessed: detallesInsertados.length,
                
                // Payment information
                transactionReference: transactionReference,
                wompiTransactionId: wompiResponse.data.idTransaccion,
                paymentAmount: wompiResponse.data.monto,
                paymentRedirectUrl: wompiResponse.data.urlCompletarPago3Ds,
        ...
        console.error(`❌ [${requestId}] Error en pedido con pago Wompi:`, error.message);
        ...
        return res.status(500).json({
            success: false,
            message: 'Error creando pedido con pago',
            error: error.message,
            request_id: requestId,
            processing_time_ms: processingTime
        });
        ...
        console.log(`🏁 [${requestId}] ===== FIN PEDIDO CON PAGO WOMPI =====\n`);
    }
};
