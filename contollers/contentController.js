...
const getCategoryId = (categoryName, callback) => {
    pool.query('SELECT id FROM categorias WHERE nombre = ?', [categoryName], (err, results) => {
        if (err) {
            callback(err, null);
            return;
        }
    ...
    upload(req, res, function(err) {
        ...
        try {
            ...
            getCategoryId(categoria, (err, idcategoria) => {
                ...
                pool.query(
                    'INSERT INTO productos (titulo, descripcion, precio, porciones, seccion, idcategoria, activo, imagen, fecha_creacion, fecha_actualizacion) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
